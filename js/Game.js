class Game {
  ...
  start() {
    ...
    var obstaclesPositions = [
      { x: width - 800, y: height / 2 + 250, image: obstacle2Image },
      { x: width  - 1300 , y: height/ 2 - 150, image: obstacle1Image },
      { x: width  - 1800, y: height / 2 + 250, image: obstacle3Image },
      { x: width  - 2300 , y: height/ 2 - 180, image: obstacle4Image },
      { x: width- 2800 , y: height/ 2 , image: obstacle2Image },
  ...
  play() {
    ...
    player.getMermaidsAtEnd();

    if (allPlayers !== undefined) {
      image(track, -width * 5, 0,  width * 6, height);
      ...
      for (var plr in allPlayers) {
        ...
        if(currentLife <= 0){
          mermaids[index-1].changeImage("blast");
          mermaids[index-1].scale = 0.3
        }

        mermaids[index - 1].position.x = x;
        mermaids[index - 1].position.y = y;
      ...
      if (player.positionY > finshLine) {
        gameState = 2;
        player.rank += 1;
        Player.updatemermaidsAtEnd(player.rank);
        player.update();
        this.showRank();
      }
  ...
  handleMermaidACollisionwithMermaidrB(index){
    ...
    if(index === 2){
      if(mermaids[index-1].collide(mermaids[0])){
        if(this.leftKeyActive){
          player.positionX += 100
        }
        else{
  ...
  handleObstacleCollision(index){
    if(mermaids[index-1].collide(obstacles)){
  ...
  handleFuel(index) {
    // Adding fuel
    mermaids[index - 1].overlap(fuels, function(collector, collected) {
      player.fuel = 185;
  ...
  handlePowerCoins(index) {
    mermaids[index - 1].overlap(powerCoins, function(collector, collected) {
      player.score += 21;
      player.update();
      //collected is the sprite in the group collectibles that triggered
      //the event
